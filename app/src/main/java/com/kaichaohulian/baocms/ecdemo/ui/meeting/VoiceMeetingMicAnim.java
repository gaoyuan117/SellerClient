...
import com.kaichaohulian.baocms.ecdemo.ui.chatting.base.emoji.Objects;
...
public class VoiceMeetingMicAnim extends LinearLayout implements Runnable {
    ...
    synchronized void  initBottomStatus(int num){//4
        ...
        for (int i = 0; i < 6; i++) {
            ...
            if(i > (6 - num - 1)) {//1
                imageViewl_i.setImageResource(R.mipmap.chatroom_speaker);
            } else {
                imageViewl_i.setImageResource(R.mipmap.chatroom_unspeaker);

            }
            if(i >= num) {//4
                imageViewR_i.setImageResource(R.mipmap.chatroom_unspeaker);
            } else {
                imageViewR_i.setImageResource(R.mipmap.chatroom_speaker);
            }
